...
| `packages/shared/src/db.ts` | DB抽象化レイヤー（SQLite/PostgreSQL対応） |
| `packages/shared/src/claude.ts` | Claude API統合（LP自動生成） |
| `packages/shared/src/sns.ts` | SNS投稿機能（Twitter API v2） |
...
| `apps/admin/app/auth.ts` | NextAuth.js認証設定 |
...
- テーブル:
  ...
  - `lp_configs` (id, title, description, content, genre, keywords, created_at, updated_at)
  - `sns_accounts` (id, platform, name, api_key, api_secret, access_token, access_secret, is_active)
...
- AI自動LP生成（Claude API）
- クリックログ管理・分析
- SNS自動投稿（Twitter API v2）
- 複数ジャンル・複数アカウント対応
- 管理画面認証（NextAuth.js v5）
- DB抽象化（SQLite/PostgreSQL）
- Vercelデプロイスクリプト
- 包括的な運用マニュアル（README.md）
